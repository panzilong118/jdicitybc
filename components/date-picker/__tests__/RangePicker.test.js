...
describe('RangePicker', () => {
  ...
  it('switch to corresponding month panel when click presetted ranges', () => {
    ...
    rangeCalendarWrapper.find('.jc-calendar-range-quick-selector a')
      .simulate('click');
    expect(render(wrapper.find('Trigger').instance().getComponent()))
      .toMatchSnapshot();
  });
  ...
  it('highlight range when hover presetted range', () => {
    ...
    expect(rangeCalendarWrapper.find('.jc-calendar-selected-day').length).toBe(2);
  });
  ...
  it('should trigger onCalendarChange when change value', () => {
    ...
    const rangeCalendarWrapper = mount(wrapper.find('Trigger').instance().getComponent());
    rangeCalendarWrapper.find('.jc-calendar-cell').at(15).simulate('click');
  ...
  it('should not throw error when value is reset to `[]`', () => {
    ...
    const rangeCalendarWrapper = mount(wrapper.find('Trigger').instance().getComponent());
    expect(() => rangeCalendarWrapper.find('.jc-calendar-cell').at(15).simulate('click').simulate('click'))
      .not.toThrow();
  });
  ...
  it('should not throw error when select after clear', () => {
    ...
    let rangeCalendarWrapper = mount(wrapper.find('Trigger').instance().getComponent());
    rangeCalendarWrapper.find('.jc-calendar-cell').at(15).simulate('click').simulate('click');
    wrapper.update();
  ...
  it('clear hover value after panel close', () => {
    ...
    wrapper.find('.jc-calendar-picker-input').simulate('click');
    wrapper.find('.jc-calendar-cell').at(25).simulate('click');
    wrapper.find('.jc-calendar-cell').at(27).simulate('mouseEnter');
    document.dispatchEvent(new MouseEvent('mousedown'));
    jest.runAllTimers();
    wrapper.find('.jc-calendar-picker-input').simulate('click');
    expect(
      wrapper.find('.jc-calendar-cell').at(23).hasClass('jc-calendar-in-range-cell')
    ).toBe(true);
  });
  ...
  describe('preset range', () => {
    it('static range', () => {
      ...
      wrapper.find('.jc-calendar-picker-input').simulate('click');
      wrapper.find('.jc-calendar-range-quick-selector a').simulate('click');
      expect(
        wrapper.find('.jc-calendar-range-picker-input').first().getDOMNode().value
      ).toBe(range[0].format(format));
      expect(
        wrapper.find('.jc-calendar-range-picker-input').last().getDOMNode().value
  ...
  it('input date manually', () => {
    ...
    const input = wrapper.find('.jc-calendar-input').first();
  ...
  it('invali end date not throw error', () => {
    ...
    wrapper.find('.jc-calendar-picker-input').simulate('click');
    expect(() =>
      wrapper.find('.jc-calendar-input').at(1).simulate('change', { target: { value: '2016-01-01' } })
    ).not.toThrow();
  });
});
