...
describe('Table.pagination', () => {
  ...
  it('should not show pager if pagination.hideOnSinglePage is true and only 1 page', () => {
    ...
    expect(wrapper.find('.jc-pagination')).toHaveLength(1);
    ...
    expect(wrapper.find('.jc-pagination')).toHaveLength(0);
  ...
  it('should have pager when change pagination from false to undefined', () => {
    ...
    expect(wrapper.find('.jc-pagination')).toHaveLength(1);
    expect(wrapper.find('.jc-pagination-item-active')).toHaveLength(1);
  });
  ...
  it('should display pagination as prop pagination change between true and false', () => {
    const wrapper = mount(createTable());
    expect(wrapper.find('.jc-pagination')).toHaveLength(1);
    expect(wrapper.find('.jc-pagination-item')).toHaveLength(2);
    ...
    expect(wrapper.find('.jc-pagination')).toHaveLength(1);
    expect(wrapper.find('.jc-pagination-item')).toHaveLength(2);
    wrapper.find('.jc-pagination-item-2').simulate('click');
    expect(renderedNames(wrapper)).toEqual(['Tom', 'Jerry']);
    wrapper.setProps({ pagination: false });
    expect(wrapper.find('.jc-pagination')).toHaveLength(0);
    wrapper.setProps({ pagination: true });
    expect(wrapper.find('.jc-pagination')).toHaveLength(1);
    expect(wrapper.find('.jc-pagination-item')).toHaveLength(1); // pageSize will be 10
  ...
  it('change to correct page when data source changes', () => {
    ...
    wrapper.find('.jc-pagination-item-3').simulate('click');
    wrapper.setProps({ dataSource: [data[0]] });
    expect(wrapper.find('.jc-pagination-item-1').hasClass('jc-pagination-item-active')).toBe(true);
  });

  it('specify the position of pagination', () => {
    const wrapper = mount(createTable({ pagination: { position: 'top' } }));
    expect(wrapper.find('.jc-spin-container').children()).toHaveLength(2);
    expect(wrapper.find('.jc-spin-container').childAt(0).find('.jc-pagination')).toHaveLength(1);
    wrapper.setProps({ pagination: { position: 'bottom' } });
    expect(wrapper.find('.jc-spin-container').children()).toHaveLength(2);
    expect(wrapper.find('.jc-spin-container').childAt(1).find('.jc-pagination')).toHaveLength(1);
    wrapper.setProps({ pagination: { position: 'both' } });
    expect(wrapper.find('.jc-spin-container').children()).toHaveLength(3);
    expect(wrapper.find('.jc-spin-container').childAt(0).find('.jc-pagination')).toHaveLength(1);
    expect(wrapper.find('.jc-spin-container').childAt(2).find('.jc-pagination')).toHaveLength(1);
  });
});
