...
describe('message', () => {
  ...
  it('should be able to config top', () => {
    ...
    expect(document.querySelectorAll('.jc-message')[0].style.top).toBe('100px');
  });
  ...
  it('should be able to hide manually', () => {
    ...
    expect(document.querySelectorAll('.jc-message-notice').length).toBe(2);
    hide1();
    jest.runAllTimers();
    expect(document.querySelectorAll('.jc-message-notice').length).toBe(1);
    hide2();
    jest.runAllTimers();
    expect(document.querySelectorAll('.jc-message-notice').length).toBe(0);
  });

  it('should be able to destroy globally', () => {
    message.info('whatever', 0);
    message.info('whatever', 0);
    expect(document.querySelectorAll('.jc-message').length).toBe(1);
    expect(document.querySelectorAll('.jc-message-notice').length).toBe(2);
    message.destroy();
    expect(document.querySelectorAll('.jc-message').length).toBe(0);
    expect(document.querySelectorAll('.jc-message-notice').length).toBe(0);
  });
  ...
  // https://github.com/jc-design/jc-design/issues/8201
