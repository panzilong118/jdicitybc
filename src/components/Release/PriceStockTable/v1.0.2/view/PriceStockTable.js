...
import { Form,Button,Layout,Modal,Checkbox,Table,Col,Row,Tooltip,
...
export default class PriceStockTable extends Component {
  constructor(props){
    ...
    const skuPriceDescTip = "sku商品的价格描述信息，将会展示在商详页sku列表中原商品价格处";
    this.columns = [
    ...
    this.columns2 = [
      ...
      {
        title: <span>价格描述 <Tooltip placement="top" title={skuPriceDescTip}><Icon style={{opacity:"0.5"}} type="question-circle" /></Tooltip></span>,
        key: 'priceDesc',
        width: '100',
        render:(text, record, index) => {
          return(
            <Input  onChange = {(e)=>this.handleChanegPriceDesc(e,index)}
                    value = {this.props.itemTmplPublishVo.itemPerfectVo.preSkuPriceVoList[index] && 
                      this.props.itemTmplPublishVo.itemPerfectVo.preSkuPriceVoList[index].priceDescVo ?
                      this.props.itemTmplPublishVo.itemPerfectVo.preSkuPriceVoList[index].priceDescVo.priceDesc : null}
    ...
    let columnsSku2 = [
      ...
      {
        title: <span>价格描述 <Tooltip placement="top" title={skuPriceDescTip}><Icon type="question-circle" /></Tooltip></span>,
        key: 'priceDesc',
        render:(text, record, index) => {
          return(
            <Input  onChange = {(e)=>this.handleChanegPriceDesc(e,index)}
    ...
    !this.props.ifProposalPrice && columnsSku2.splice(1, 1);
    this.columnsSku = columnsSku;
    this.columnsSku2 = columnsSku2;
    this.initdata=[{
      key: 0,
      count: 0,
      totalPrice: null,//供货价
    ...
    this.lastKey = 0;
    this.record = {};
    this.showSaleType = false; //判断是否展示询价 描述区间价输入框
  }
      ...
      this.props.queryShopInfo(param).then(
        result => {
          if(result.code == 0 && result.data && result.data.length > 0) {
            ...
            this.columns2.splice(-1, 0, html);
            this.columnsSku2.splice(-1, 0, html2);
            this.setState({
