...
import "./style/operatingOnlyTop.css";
...
export default class OperatingOnlyTop extends Component {
  ...
  render() {
    ...
    let {hKey,vName, ...rest} = verticalMenu;

    return (
      <LocaleProvider locale={zh_CN}>
        <div style={{minHeight: this.minHeight + "px"}} className='operating-layout-onlytop'>
          <Helmet>
              {vName && vName.length>0? <title>{vName}</title> :""}
              <link rel="icon" href={this.state.icon} mce_href={this.state.icon} type="image/x-icon" />
          </Helmet>
          <div className='operating-layout-right-onlytop'>
            <div style={{background: '#fff', height: 'auto', padding: '0'}}>
              <OperatingHeader menus={horizontalMenu} selectKey={hKey} logo={this.state.logo}/>
            </div>
